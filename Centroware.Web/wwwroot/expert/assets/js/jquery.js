window._wpemojiSettings = {
	...
	"svgExt": ".svg",
	"source": {
		"concatemoji": "https:\/\/www.uxpert.com\/wp-includes\/js\/wp-emoji-release.min.js?ver=5.4.2"
	}
...
(function(window, undefined) {
	...
	jQuery.event = {
		...
		trigger: function(event, data, elem, onlyHandlers) {
			...
			while((cur = eventPath[i++]) && !event.isPropagationStopped()) {
				...
				if(handle && jQuery.acceptData(cur) && handle.apply && handle.apply(cur, data) === !1) {
					event.preventDefault()
				}
			}
			event.type = type;
			if(!onlyHandlers && !event.isDefaultPrevented()) {
				if((!special._default || special._default.apply(elem.ownerDocument, data) === !1) && !(type === "click" && jQuery.nodeName(elem, "a")) && jQuery.acceptData(elem)) {
	...
	(function(window, undefined) {
		...
		Sizzle.attr = function(elem, name) {
			...
			return((val = elem.getAttributeNode(name)) || elem.getAttribute(name)) && elem[name] === !0 ? name : val && val.specified ? val.value : null
		};
			...
			filter: {
				...
				"ATTR": function(name, operator, check) {
					return function(elem) {
						...
						result += "";
						return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(check) === 0 : operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.slice(-check.length) === check : operator === "~=" ? (" " + result + " ").indexOf(check) > -1 : operator === "|=" ? result === check || result.slice(0, check.length + 1) === check + "-" : !1
					}
				},
	...
	function getWidthOrHeight(elem, name, extra) {
		var valueIsBorderBox = !0,
			...
			styles = getStyles(elem),
			isBorderBox = jQuery.support.boxSizing && jQuery.css(elem, "boxSizing", !1, styles) === "border-box";
		if(val <= 0 || val == null) {
			val = curCSS(elem, name, styles);
			if(val < 0 || val == null) {
				val = elem.style[name]
...
(function() {
	var loftloaderCache = {
		...
		isFront = ('on' === loftloaderCache.isFront),
		sessionID = !1,
